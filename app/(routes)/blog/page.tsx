...
export default function Blog() {
  ...
  return (
    <main>
      <section className="relative lg:px-12 w-11/12 mx-auto 2xl:gap-x-6 mt-10 lg:mt-16 xl:mt-32 mb-44 flex flex-col gap-5 lg:gap-10">
        <h1
          className={`${albert.className} text-4xl md:text-5xl lg:text-6xl font-bold text-main`}
        >
          Blog
        </h1>
        <div className="flex flex-col md:grid md:grid-cols-2 lg:grid-cols-[2fr,1fr] grid-rows-2 gap-8 mt-5 xl:h-[650px]">
          <HighlightedPost post={posts[0]} />
          <RegularPost post={posts[1]} listed={false} />
          <RegularPost post={posts[2]} listed={false} />
        </div>
        <div className="flex flex-col md:grid-cols-2 md:grid grid-cols-3 gap-8 mt-10">
          <h2
